...
%D:\Exports for manipulation\18wk__3kHz  -- file dir external hard drive
in_data_dir = 'C:\Users\luzsa\OneDrive - University of Pittsburgh\Shared Documents - SHRS Partha Lab\Data\Animal_Data\Analysis\AgingSeries_Gerbils\dAM Aging Exports\75wk\ABR\3kHz\'; % path to binary files (input)
data_out_dir = 'C:\Users\luzsa\OneDrive - University of Pittsburgh\Shared Documents - SHRS Partha Lab\Data\Animal_Data\Analysis\AgingSeries_Gerbils\dAM Aging Data\75wk\ABR\'; % where output ABR values will be saved 

% in_data_dir = 'C:\Users\ASP97\University of Pittsburgh\SHRS Partha Lab - Documents\Data\Animal_Data\Analysis\AgingSeries_Gerbils\dAM Aging Exports\75wk\ABR\500Hz\'; % path to binary files (input)
% data_out_dir = 'C:\Users\ASP97\University of Pittsburgh\SHRS Partha Lab - Documents\Data\Animal_Data\Analysis\AgingSeries_Gerbils\dAM Aging Data\75wk\500Hz\'; % where output ABR values will be saved 
...
fname_out= [data_out_dir 'ABR_GAs'];
if ~isfolder(data_out_dir)
    mkdir(data_out_dir)
end
...
filttype = 1; % whether to filter


allfiles= dir([in_data_dir 'GER*_E1']); % calls all data with filename including GER and E1 (change this for averaging across different montages)

fs = 24414.0625; %TDT sampling frequency 

lp = 3000; hp = 80; order=4;  %Check if this is OK

tvec=1/fs:1/fs:500/fs;%Time domain vector at sampling frequency
...
for fileVar=1:length(allfiles)
    fileVar
        ...
        [bh,ah]=butter(order,hp/(fs*0.5),'high'); %butterworth filter
...
ABR_GAstd_sum(:,j) = std(allfiles_tdatsum(:,j:9:size(allfiles_tdatsum,2)),0,2);
...
for i = 1:9 
    subplot(9,1,i)
    shadedErrorBar(tvec(1:length(ABR_GA_sum(:,i))),ABR_GA_sum(:,i),ABR_GAstd_sum(:,i));
%     plot(ABR_GA_sum(:,i))
    xlim ([0 0.02]) %loking at 20 ms 
%     ylim ([-0.15 0.15]); %Equal y axis
hold off;
end 
...
if do_save_data
    save(fname_out, 'ABR_GA_sum', 'ABR_GAstd_sum');
end 
if do_save_fig
    print(fname_out, '-dpng', '-r600')
    savefig(fname_out);
end
